...
#check first 6 rows of each dataframe
df.head(6)
df2.head(6)
...
df2['url'][0]
#split new_url by '=' and extract the second item, which is id, and store them into new_id
df2['df2_id'] = df2['url'].str.split('=').str[1]
df2['df2_id'][0]
#covert df2_id and id to float type in order to match with each other
df2['df2_id'] = df2['df2_id'].astype(float)
df['df1_id'] = df['id'].astype(float)
#inner join df and df2, on df['id'] and df['new_id']
df3 = pd.merge(df, df2, how='inner', left_on = 'df1_id', right_on = 'df2_id')
df3.head(10)

#covert other byte types to string
